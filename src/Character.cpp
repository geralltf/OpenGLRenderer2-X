...
void Character::Initilise()
{
	...
	for (int i = 0; i < numPointLights; i++)
	{
		...
		light->position = Vector3f::Scale(new Vector3f(PointLight::RandomFloat(), PointLight::RandomFloat(), PointLight::RandomFloat()), 10);
		light->aabb = new AABB(light->position, Vector3f::Scale(Vector3f::ONE, 0.1f));

		light->constant = 0.01f;
		light->linear = 0.001f;
		light->quadratic = 0.001f;
		light->ambient = new Vector3f(0, 0, 0);
		light->diffuse = new Vector3f(PointLight::RandomFloat01(), PointLight::RandomFloat01(), PointLight::RandomFloat01());
		light->specular = new Vector3f(PointLight::RandomFloat01(), PointLight::RandomFloat01(), PointLight::RandomFloat01());
		light->intensity = PointLight::RandomFloat01();
		light->transform = Matrix4::Identity();
		light->isActive = 1;
...
void Character::Render(sf::RenderWindow* window, Transform* cameraTransform, Matrix4* projectionMatrix, Matrix4* modelview, Vector3f* light_dir, Matrix4* lightModelView)
{
	bool outlinePass = true;
...
void Character::RenderModel(sf::RenderWindow* window, Transform* cameraTransform, Matrix4* projectionMatrix, Matrix4* modelview, Vector3f* light_dir, bool outlinePass)
{
	...
	for (int i = 0; i < pointLights->size(); i++)
	{
		...
		float smoothing = 1.1;
		light->transform->Translate(Vector3f::Scale(PointLight::RandomVector(), smoothing));
		Vector3f* pos = light->transform->TransformPosition(light->position);
	...
	for (int i = 0; i < pointLights->size(); i++)
	{
		Vector3f* diffuse_lightcolour = (*pointLights)[i]->diffuse;
		Matrix4* light_transform = (*pointLights)[i]->transform;
		PointLight* point_light = (*pointLights)[i];
		point_light->aabb->Debug(new ColourRGBA(new Vector4f(diffuse_lightcolour->x, diffuse_lightcolour->y, diffuse_lightcolour->z, 1.0f)));
		Debug::DrawLines_RenderDispatch(window, cameraTransform, projectionMatrix, Matrix4::Multiply(modelview, light_transform));
	}
}
...
void Character::RenderShadowMap(sf::RenderWindow* window, Transform* cameraTransform, Matrix4* modelview, Vector3f* light_dir, Matrix4* lightModelView)
{
	...
	Matrix4* depthViewMatrix = Matrix4::LookAt(light_dir, new Vector3f(0.0f, 0.0f, 0.0f), Vector3f::Up);
	depthMVP = Matrix4::Multiply(Matrix4::Multiply(depthProjectionMatrix, depthViewMatrix), lightModelView);
	float* arrDepthMVP = depthMVP->ToArray();
...
void Character::RenderAOMap(sf::RenderWindow* window, Transform* cameraTransform, Matrix4* modelview, Vector3f* light_dir, Matrix4* lightModelView)
{
	...
	Matrix4* aoViewMatrix = Matrix4::LookAt(Vector3f::Add(new Vector3f(0.0f, 1.0f, 0.0f), light_dir), new Vector3f(0.0f, 0.0f, 0.0f), Vector3f::Up);
	...
	Matrix4* aoMVP = Matrix4::Multiply(Matrix4::Multiply(aoProjectionMatrix, aoViewMatrix), lightModelView);
